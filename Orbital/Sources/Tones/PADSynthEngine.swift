...
final class PADSynthEngine {
  ...
  /// Uses middle C (261.63 Hz) as the reference pitch for display.
  func recomputeDisplay() async {
    let fundamentalHz: CoreFloat = 261.63
    let rawFreqAmp = generateFreqAmp(fundamentalHz: fundamentalHz)
    let envelopedFreqAmp = generateEnvelopedFreqAmp(fundamentalHz: fundamentalHz)

    freqAmp = rawFreqAmp
    displayFreqAmp = downsampleForDisplay(rawFreqAmp)

    // Scale envelope and product to match the blue spectrum's peak amplitude
    let blueMax = displayFreqAmp.map(\.amplitude).max() ?? 1.0
    displayEnvelope = computeEnvelopeDisplay(scale: blueMax)
    displayProduct = downsampleForDisplay(envelopedFreqAmp.map { $0 * blueMax })
  }
}
