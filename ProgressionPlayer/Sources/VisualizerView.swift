...
<body>
  ...
  <div class="start-overlay" id="startOverlay">
    <h1>Butterchurn</h1>
    <p>WebGL music visualizer powered by Milkdrop. Tap to start visualizing.</p>
  ...
  <div class="controls" id="controls">
    ...
    <div class="controls-row">
      <button id="randomBtn" title="Random Preset">
        <svg viewBox="0 0 24 24"><path d="M10.59 9.17L5.41 4 4 5.41l5.17 5.17 1.42-1.41zM14.5 4l2.04 2.04L4 18.59 5.41 20 17.96 7.46 20 9.5V4h-5.5zm.33 9.41l-1.41 1.41 3.13 3.13L14.5 20H20v-5.5l-2.04 2.04-3.13-3.13z"/></svg>
        <span>Random</span>
      </button>
      <button id="cycleBtn" title="Auto-cycle Presets">
      ...
      <button id="hideBtn" title="Hide Controls">
  ...
  <script type="module">
    ...
    // Receive samples from Swift
    window.pushSamples = (samples) => {
        // samples is Array of numbers
        audioQueue.push(...samples);
        // Limit queue size to prevent memory explosion if not consumed
        if (audioQueue.length > 44100 * 2) {
            audioQueue.splice(0, audioQueue.length - 44100);
        }
    };
    ...
    function initPresets() {
      ...
      presetKeys = Object.keys(presets).sort((a, b) => 
    ...
    // Load a preset
    function loadPreset(name, blendTime = 1.0) {
    ...
    function randomPreset() {
      if (!presetKeys || presetKeys.length === 0) return;
    ...
    function toggleCycle() {
      ...
      if (isCycling) {
        cycleInterval = setInterval(randomPreset, 15000);
      } else {
    ...
    // Show controls on tap (when hidden)
    ...
    async function start() {
      ...
      try {
        ...
        // Create audio context (must be after user gesture for iOS)
        ...
        // Create ScriptProcessor to act as source
        ...
        processor.onaudioprocess = (e) => {
            ...
            if (audioQueue.length >= BUFFER_SIZE) {
                const chunk = audioQueue.splice(0, BUFFER_SIZE);
                output.set(chunk);
            } else {
                // Not enough data, fill with silence to keep graph running
                // Or maybe stretch? Silence is safer for now.
                output.fill(0);
                // If we have some data but less than buffer, use it?
                if (audioQueue.length > 0) {
                    const chunk = audioQueue.splice(0, audioQueue.length);
                    output.set(chunk, 0);
                }
            }
        };
        
        // We need to connect processor to destination for it to run?
        // Web Audio API requirement: script processor must be connected to destination 
        // OR have a connected output to be active.
        // But we don't want to HEAR it (it's already playing natively).
        // Connect to a gain node with 0 gain.
        ...
        // Connect our processor to visualizer
        visualizer.connectAudio(processor);
        
        // Load initial preset
        initPresets();
        randomPreset();
        
        // Hide overlay, start rendering
        ...
        let msg = 'Error starting visualizer: ' + err.message;
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'r') randomPreset();
      if (e.key === 'c') toggleCycle();
      if (e.key === 'h' || e.key === 'Escape') toggleControls();
      if (e.key === 'ArrowRight') {
        const idx = (presetKeys.indexOf(currentPreset) + 1) % presetKeys.length;
        loadPreset(presetKeys[idx]);
      }
      if (e.key === 'ArrowLeft') {
        const idx = (presetKeys.indexOf(currentPreset) - 1 + presetKeys.length) % presetKeys.length;
        loadPreset(presetKeys[idx]);
      }
    });
...
"""#
    ...
    class Coordinator {
        ...
        let updateInterval: TimeInterval = 1.0 / 30.0 // Limit to 30fps for bridge safety
        ...
        func setupAudioTap(webView: WKWebView) {
            ...
            synth.engine.installTap { [weak self] samples in
                ...
                let jsonString = samples.description
